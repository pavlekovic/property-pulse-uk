...
c1, c2, c3, spacer, c4, c5, c6= st.columns([1, 1, 1, 0.5, 1, 1, 1])
with c1:
...
with c2:
    ...
    if district:
        # If you don't have district_to_postcodes in your artifact, just skip this filtering
        district_to_postcodes = art.get("district_to_postcodes", {})
        if district_to_postcodes:
            valid_postcodes = sorted(district_to_postcodes.get(district, []))
        else:
            # fallback: let user pick from *all* keys in postcode_map
            valid_postcodes = sorted(art.get("postcode_map", {}).keys())

    # Always prepend a placeholder
    full_postcode = st.selectbox(
        "Postcode",
        options=(["Select a postcode"] + valid_postcodes),
        index=0
    )

    if full_postcode == "Select a postcode":
        postcode_zone = None
    else:
        postcode_zone = art.get("postcode_map", {}).get(full_postcode)
with c3:
...
# ---------- Forecast ----------
key = (district, postcode_zone, ptype, new_build, tenure)
...
if district is not None and postcode_zone is not None and asking_price is not None:
    ...
    with col1:
        if seg_now is not None and growth_pct is not None:
            ...
            st.info("Could not compute projection for this selection.")
